  ...
  return (
    <div className="rounded-2xl border border-border/50 glass p-8 shadow-xl shadow-primary/5">
      <h3 className="text-sm font-black text-text mb-6 flex items-center gap-3 uppercase tracking-widest">
        ...
        Questions viewers can ask
      </h3>
      <div className="space-y-4">
        {questions.map((q, idx) => (
          <div
            key={idx}
            className={`group rounded-xl border transition-all duration-300 ${expandedIndex === idx
          ...
          >
            ...
            >
              <span className={`flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full font-bold text-sm mt-0.5 transition-colors ${expandedIndex === idx
            ...
            <div
              className={`overflow-hidden transition-all duration-300 ease-in-out ${expandedIndex === idx ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
                }`}
            >
